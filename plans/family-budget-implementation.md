...
SHA/доказательство: implementation `658d3b8`; точный pathspec из `5` файлов без этого плана: `tools/build-family-budget.mjs`, `docs/EXCEL_MVP.md`, `docs/IMPLEMENTATION_STATUS.md`, `outputs/2026-07-29-phase2-rc1/family-budget-mvp.xlsx`, `outputs/2026-07-29-phase2-rc1/family-budget-mvp.xlsx.inspect.ndjson`. Требуемый Spreadsheet runtime загружен: Node `24.14`, `@oai/artifact-tool` доступен; генератор использует семантику канонического G-002. Итоговая книга содержит `11` листов и `6` таблиц; XLSX — `48 795` байт, SHA-256 `478e173ce41f9231c84c5831ebf0704347cdfa81d09b6eefe87430b4056e1ed9`; компактный inspection — `12 428` байт, SHA-256 `da2125b38b934bdd27d50fbc1226d94aed372cd9d4059fe901319d1d1558cb8a`. Финальный sanitized XLSX повторно импортирован: совпадений formula errors `#REF!/#DIV/0!/#VALUE!/#NAME?/#N/A` — `0`; в архиве `34` записи, VBA/macros, external links/connections и локальные пути отсутствуют; подтверждены `8` типизированных дат, `17` денежных значений и `15` validation rules. Все `11` preview-листов визуально прошли проверку без обрезания; временные previews не коммитились. Точный G-002 live-check: июль — доход `180 000 ₽`, ежемесячные `53 000 ₽`, повседневные `53 000 ₽`, резерв `19 809 ₽`, цель `10 000 ₽`, свободно `44 191 ₽`; сентябрь — сезонное `31 000 ₽`, свободно `13 191 ₽`; январь 2027 — due `72 000 ₽`, свободно `13 191 ₽`, без повторного вычитания. Проверки построчного `ROUNDUP` после первой строки: `72 000 → 72 001 ₽` меняет месячный резерв `6 000 → 6 001 ₽` и общий резерв `15 523 → 15 524 ₽`; новая строка `12 001 ₽` даёт `1 001 ₽`. Mutation/rollback пройден для всех `6` таблиц; отрицательный symlink-probe отклонён. `pnpm verify` — `302/302` теста, все typechecks и build зелёные; `pnpm audit` и `pnpm audit --prod` — `0` vulnerabilities. Финальные свежие code/formula review и security review — `APPROVE`, blocker/high/medium/low `0/0/0/0` каждое. Зафиксированный дефолт: листы намеренно не защищены, чтобы пользователь мог редактировать книгу; это не считается security-контролем. Книга явно не зашифрована. Ручная проверка в реальном Excel через Accessibility Checker, клавиатуру и масштаб `200%` остаётся открытой.
...
- [x] Явный blocker: не начинать генерацию и не закрывать фазу, пока недоступен требуемый artifact-tool loader/runtime для Spreadsheet workflow. Недоступность loader фиксируется как blocker, а не обходится повторной публикацией старого workbook или неподтверждённым альтернативным генератором.
- [x] Генератор берёт значения/семантику из канонического G-002 или явного общего адаптера, а не поддерживает вторую независимую арифметику fixtures.
- [x] Повторно сгенерировать 11 листов: быстрый старт, дашборд, горизонт 24 мес, крупные и ежегодные, ежемесячные, сезонные, повседневные, операции, цели, настройки, справочники.
- [x] Проверить типы дат/денег, validation, расширение таблиц новой строкой, тексты статусов, подписи графика и отсутствие внешних ссылок/макросов.
- [x] Запустить команду генерации по инструкции `spreadsheets` и формульный scan; найдено `0` совпадений `#REF!/#DIV/0!/#VALUE!/#NAME?/#N/A`.
- [x] Live-check книги: 11 листов; июль доход `180 000 ₽`, ежемесячные `53 000 ₽`, повседневные `53 000 ₽`, резерв `19 809 ₽`, цель `10 000 ₽`, свободно `44 191 ₽`; сентябрь сезонное `31 000 ₽`, свободно `13 191 ₽`; январь 2027 платёж из резерва `72 000 ₽` без второго вычитания.
- [x] Свежий code/formula review проверяет формулы, structured ranges и parity с G-002.
- [x] Свежий security review проверяет отсутствие macros/external links, ложного заявления о шифровании и утечки локальных путей/данных в артефакт.
- [x] Коммит A содержит только генератор, новый итоговый `.xlsx`, минимальный inspection и docs; временные PNG previews не коммитить. Коммит B содержит только план с SHA/доказательством A.
