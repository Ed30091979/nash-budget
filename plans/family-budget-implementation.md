...
- [ ] Первый запуск не сохраняет демонстрационные финансовые данные; demo доступно только как явно подписанный необязательный пример/import fixture.
- [ ] Четыре коротких шага: состав семьи/локальные метки, RUB и период, доход/обязательные платежи, 3–5 повседневных лимитов.
- [ ] Все поля имеют понятные подписи, back/forward без потери ввода, validation и touch targets не меньше 44 CSS px.
- [ ] `pnpm --filter @family-budget/web test`, typecheck, production build и `pnpm verify` зелёные.
- [ ] Live-check чистого профиля: до onboarding `0` операций и `0` demo-платежей; ввести доход `180 000 ₽`, обязательный платёж `53 000 ₽`, повседневный лимит `53 000 ₽`; после завершения остаётся `0` фактических операций, период начинается `2026-07-01`, дашборд показывает нераспределённо `74 000 ₽` до добавления резервов/целей.
- [ ] Reload и offline reload возвращают тот же созданный бюджет и те же три суммы.
- [ ] Свежий code review проверяет state machine, validation, focus/errors и отсутствие скрытого seed.
- [ ] Свежий security review проверяет XSS через имена/категории, PII minimization, отсутствие данных в URL/logs/cache.
- [ ] Коммит A содержит только web-файлы фазы; коммит B содержит только план с SHA/доказательством A.
...
- [ ] Создание, редактирование и удаление income, expense, refund, transfer и goal contribution.
- [ ] Перевод и взнос в цель записываются атомарно; same-account запрещён; refund связан с категорией/исходной операцией; использованная категория архивируется.
- [ ] После любой операции пересчитываются карточки, история и статусы `normal / near_limit / exhausted / over_limit`; сигнал содержит текст, значок и число.
- [ ] `pnpm verify` и integration-тесты IDB + domain зелёные.
- [ ] Live-check G-000: старт `4` операций, доход `100 000 ₽`, расходы `76 500 ₽`, продукты `31 500 ₽` и перелимит `1 500 ₽`.
- [ ] Добавить refund `1 500 ₽` от `2026-07-07`: операций `5`, расходы `75 000 ₽`, продукты `30 000 ₽`, статус `Лимит исчерпан`, перелимит `0 ₽`.
- [ ] Добавить transfer `5 000 ₽`: операций `6`, расходы остаются `75 000 ₽`, капитал `25 000 ₽`, основной `20 000 ₽`, накопительный `5 000 ₽`; edit/delete полностью возвращают прежние числа.
- [ ] Проверить отдельно 80%, 100%, 100% + `0,01 ₽` и нулевой план без crash.
- [ ] Свежий code review проверяет атомарность, edit/delete recalculation и idempotency UI-submit.
- [ ] Свежий security review проверяет replay/double-submit, malformed IDs, oversized notes, data exposure в errors/logs и CSV/backup взаимодействие.
- [ ] Коммит A содержит только operation/integration-файлы; коммит B содержит только план с SHA/доказательством A.
